...
Brain Mask · Acquisition + Hyper-parameters · Run · Log · Echo Selection ·
...
def _print_run_config(work_dir, mode, phase_paths, te_list_ms, mag_path, mask_path,
                      voxel_size, b0, eroded_rad, phase_sign, auto_bet2=True):
    print("============================")
...
def _run_thread(job, work_dir, mode, phase_paths, te_list_ms, mag_path, mask_path,
                voxel_size, b0, eroded_rad, phase_sign, run_iqfm,
                vmin, vmax, lfs_vmin, lfs_vmax, auto_bet2):
    log_q = job["log_queue"]
    orig = sys.stdout
    ...
    try:
        with _pipeline_lock:
            ...
            # iQFM only makes sense with a mask — defensively gate it.
            run_iqfm_eff = bool(run_iqfm) and (mask_path is not None)
            if run_iqfm and not mask_path:
                print("Note: iQFM disabled because no brain mask was provided.")

            per_echo_qsm_paths, per_echo_lfs_paths = [], []
            if len(phase_paths) == 1:
                ...
                for i, (ppath, te_s) in enumerate(zip(phase_paths, te_list_s)):
                    ...
                    # Save per-echo iQSM/iQFM in the main output dir with clear names
                    new_q = out_dir / f"iQSM_e{i+1}.nii.gz"
                    shutil.copy(q_path, new_q)
                    per_echo_qsm_paths.append(str(new_q))
                    if l_path:
                        lfs_maps.append(nib.load(l_path).get_fdata(dtype=np.float32))
                        new_l = out_dir / f"iQFM_e{i+1}.nii.gz"
                        shutil.copy(l_path, new_l)
                        per_echo_lfs_paths.append(str(new_l))
                    else:
                        per_echo_lfs_paths.append(None)

                print(f"\nAveraging {len(qsm_maps)} echoes …")
                qsm_stack = np.stack(qsm_maps, axis=-1)
                lfs_stack = np.stack(lfs_maps, axis=-1) if lfs_maps else None
                ...
                if mag_path:
                    ...
                    weights = te_bc ** 2  # broadcasted over the volume
            ...
            # te lists are set for both single- and multi-echo so the panel can
            # render for single-echo too (in a disabled state).
            job["per_echo_qsm_paths"] = per_echo_qsm_paths or None
            job["per_echo_lfs_paths"] = per_echo_lfs_paths or None
            ...
            job["render_lfs_vmin"]    = float(lfs_vmin)
            job["render_lfs_vmax"]    = float(lfs_vmax)
...
def _result_files(job):
    """Combined results first, then per-echo iQSM/iQFM (interleaved by echo)."""
    files = [p for p in (job.get("qsm_path"), job.get("lfs_path")) if p]
    qper = job.get("per_echo_qsm_paths") or []
    lper = job.get("per_echo_lfs_paths") or []
    n = max(len(qper), len(lper))
    for i in range(n):
        if i < len(qper) and qper[i]:
            files.append(qper[i])
        if i < len(lper) and lper[i]:
            files.append(lper[i])
...
def _build_results_zip(job):
    ...
    zip_path = Path(out_dir) / "iqsm_results.zip"
    # Overwrite any prior bundle so the URL changes when content changes.
    try:
        zip_path.unlink()
...
def _state_dict(job):
    return {
        ...
        "per_echo_lfs_paths":   job.get("per_echo_lfs_paths"),
        ...
        "render_lfs_vmin":      job.get("render_lfs_vmin"),
        "render_lfs_vmax":      job.get("render_lfs_vmax"),
    }
...
def _visibility_updates(job):
    has_lfs = bool(job.get("lfs_image"))
    ...
    return (
        ...
        gr.update(visible=has_lfs),                           # img_lfs
        gr.update(visible=has_lfs),                           # lfs_vmin_input
        gr.update(visible=has_lfs),                           # lfs_vmax_input
...
def run_pipeline(phase_files, te_ms_str, mag_files, mask_file, auto_bet2,
                 voxel_str, b0_val, eroded_rad, negate_phase, run_iqfm,
                 vmin, vmax, lfs_vmin, lfs_vmax):
    _noop = (
        None, "",                                        # files, info
        None, None,                                      # qsm_image, lfs_image
        ...
        gr.update(visible=False),                        # img_lfs
        gr.update(visible=False),                        # lfs_vmin_input
        gr.update(visible=False),                        # lfs_vmax_input
    ...
    if not phase_files:
        ...
        return
    # Magnitude is optional — multi-echo combination falls back to TE²-only weights.
    ...
    # When omitted, multi-echo combination uses TE² weights only (uniform mag).
    ...
    if mag_files_list:
        ...
        # Stage each mag file (convert .mat → NIfTI). For multi-3D, stack into 4D.
        staged_mag_paths = []
    ...
    threading.Thread(
        ...
        args=(job, work_dir, mode, phase_paths, te_list_ms, mag_path, mask_path,
              voxel_size, float(b0_val), int(eroded_rad), phase_sign, bool(run_iqfm),
              float(vmin), float(vmax), float(lfs_vmin), float(lfs_vmax), bool(auto_bet2)),
        daemon=True,
    ).start()
...
/* Phase + Magnitude upload buttons — keep them the same fixed size and
   horizontally aligned regardless of state (UploadButton's own height drifts
   based on its label). */
...
/* Hide upload zone & all clear/×-overlay buttons on processing-order widgets
   (phase + magnitude). Per-file X next to each row is preserved.
   Explicit "Remove all …" buttons handle bulk clearing. */
...
with gr.Blocks(title="iQSM", analytics_enabled=False) as app:
    ...
    gr.Markdown(
        ...
        "🐙 [GitHub](https://github.com/sunhongfu/iQSM#web-app)"
        " &nbsp;·&nbsp; "
    ...
    with gr.Column():
        ...
        with gr.Accordion("Brain Mask (optional but recommended)", open=True,
            ...
            gr.Markdown(
                "A brain mask improves **iQSM reconstruction quality** and enables "
                "**iQFM** — the local tissue field map (i.e., the background-field-"
                "removal result). If you don't upload one and magnitude is provided "
                "above, one is **generated automatically via bet2** when you click "
                "Run — uncheck the box below to skip that and reconstruct whole-head "
                "instead.\n\n"
        ...
        with gr.Accordion("Acquisition & Hyper-parameters", open=False,
            ...
            run_iqfm_cb = gr.Checkbox(
                label="Run iQFM (tissue field)",
                value=False,
                interactive=True,
                info="Reconstruct the local tissue field (LFS) in addition to QSM. "
                     "Requires a brain mask (uploaded, or bet2-auto-generated) -- "
                     "skipped (noted in the log) if none ends up being used.",
            )
        ...
        with gr.Accordion("Echo Selection (refine combination)", open=True, visible=False,
            ...
            gr.Markdown(
                ...
                "short TEs may produce artifacts in iQSM results — uncheck any echoes you "
                "want to **exclude**, then click **Recombine**. The per-echo files above are "
        ...
        with gr.Accordion("Results", open=True, visible=False,
            ...
            gr.Markdown(
                "Recombined output (`iQSM_recombined_e<…>.nii.gz`) appears first when "
                "Echo Selection has been used; the original all-echoes combination "
                "(`iQSM.nii.gz`) and per-echo files (`iQSM_e1.nii.gz`, …) follow. "
    ...
    # kept as a list and stacked into 4D at run time (mirrors phase behaviour).
    ...
    # Sort sync after manual X removal in the magnitude file widget
    ...
    def on_mask_upload(uploaded, accumulated_paths, progress=gr.Progress()):
        ...
        progress(1.0, desc="Done")
        # Both iQFM and erosion are already unlocked by default (bet2 can supply a
        # mask even without an upload) -- just reset erosion to its default here.
        return (gr.update(value=uploaded, visible=True), info, gr.update(visible=True),
                gr.update(), gr.update(value=3))
    ...
    mask_button.upload(
        ...
        outputs=[mask_file, mask_info, mask_clear_btn, run_iqfm_cb, eroded_rad],
    )

    def on_mask_change(value):
        # Only fire on transitions to "no mask"; non-None changes are no-ops.
        if value is None:
            return (gr.update(value=None, visible=False), "", gr.update(visible=False),
                    gr.update(), gr.update())
    ...
    _mask_clear_outputs = [mask_file, mask_info, mask_clear_btn, run_iqfm_cb, eroded_rad]
    ...
    _ro = [img_qsm, img_lfs, img_phase, img_mag, img_mask]
    ...
    # Recombine selected echoes — fast (no re-inference).
    def recombine_echoes(selected, state, slice_idx, vmin, vmax, lfs_vmin, lfs_vmax):
        ...
        per_lfs = state.get("per_echo_lfs_paths") or []
        te_s    = state["per_echo_te_s"]
        te_ms   = state["per_echo_te_ms"]
        out_dir = Path(state["out_dir"])

        # Load selected per-echo volumes (cached via _volume_array)
        qsm_vols = [_volume_array(per_qsm[i]) for i in idx]
        lfs_vols = []
        for i in idx:
            if i < len(per_lfs) and per_lfs[i]:
                lfs_vols.append(_volume_array(per_lfs[i]))
        # Affine from the first selected per-echo file
        affine = nib.load(per_qsm[idx[0]]).affine

        # Magnitudes for weighting
        ...
        if sel_mags:
            ...
            lfs_avg = ((weights * np.stack(lfs_vols, axis=-1)).sum(axis=-1) / denom.squeeze(-1)
                       if lfs_vols else None)
            method = "magnitude × TE² weighted"
        else:
            # No magnitude: TE² weighting only (uniform magnitude).
            weights = te_bc ** 2
            qsm_avg = (np.stack(qsm_vols, axis=-1) * weights).sum(axis=-1) / weights.sum()
            lfs_avg = ((np.stack(lfs_vols, axis=-1) * weights).sum(axis=-1) / weights.sum()
                       if lfs_vols else None)
            method = "TE² weighted (uniform magnitude)"

        # Versioned filename based on selection (and replace any prior recombination)
        # — guarantees a fresh download URL and gives provenance in the filename.
        sel_tag = "_".join(f"e{i+1}" for i in idx)
        for old in out_dir.glob("iQSM_recombined_*.nii.gz"):
            try: old.unlink()
            except OSError: pass
        for old in out_dir.glob("iQFM_recombined_*.nii.gz"):
            try: old.unlink()
            except OSError: pass

        qsm_out = out_dir / f"iQSM_recombined_{sel_tag}.nii.gz"
        ...
        lfs_out = None
        if lfs_avg is not None:
            lfs_out = out_dir / f"iQFM_recombined_{sel_tag}.nii.gz"
            nib.save(nib.Nifti1Image(lfs_avg.astype(np.float32), affine), str(lfs_out))

        # Update state — qsm/lfs_path now point at the recombined files
        new_state = dict(state)
        new_state["qsm_path"] = str(qsm_out)
        new_state["lfs_path"] = str(lfs_out) if lfs_out else None

        # Re-render mid (or current) slice
        qsm_img = _make_slice_image(str(qsm_out), slice_idx, vmin, vmax)
        lfs_img = (_make_slice_image(str(lfs_out), slice_idx, lfs_vmin, lfs_vmax)
                   if lfs_out else None)
        ...
        status = (f"✅ Recombined {len(idx)}/{len(per_qsm)} echoes via {method}.\n\n"
                  ...
                  + (f" / `{Path(lfs_out).name}`" if lfs_out else ""))

        # Refresh Results panel listing — recombined file(s) first, then the
        # original all-echoes combination, then per-echo files.
        files = [p for p in (str(qsm_out), str(lfs_out) if lfs_out else None) if p]
        original_qsm = state.get("qsm_path") if isinstance(state, dict) else None
        # state["qsm_path"] now points to the recombined file (we set it above);
        # find the original all-echoes file by name.
        all_echoes_qsm = out_dir / "iQSM.nii.gz"
        if all_echoes_qsm.exists():
            files.append(str(all_echoes_qsm))
        all_echoes_lfs = out_dir / "iQFM.nii.gz"
        if all_echoes_lfs.exists():
            files.append(str(all_echoes_lfs))
        for i in range(len(per_qsm)):
            if per_qsm[i]:
                files.append(per_qsm[i])
            if i < len(per_lfs) and per_lfs[i]:
                files.append(per_lfs[i])

        # Refresh the Download-all bundle to match the new combined files.
        zip_path = out_dir / "iqsm_results.zip"
        try: zip_path.unlink()
    ...
    recombine_btn.click(
        ...
        inputs=[echo_checkboxes, output_state, slice_slider,
                vmin_input, vmax_input, lfs_vmin_input, lfs_vmax_input],
        outputs=[recombine_status, result_file, img_qsm, img_lfs,
